...
class PHP_booklibrary {
    ...
    function save_book_fe($option) {
        ...
        if ($booklibrary_configuration['addbook_email']['show'] && trim($booklibrary_configuration['addbook_email']['address']) != "") {
            if (isset($my->id) AND $my->id != 0) {
                ...
                mail($booklibrary_configuration['lendrequest_email']['address'], $mes_title, $message, $headers);
            }
        }
...
//********************   begin add send mail for admin   ********
        ...
        if ($params->get('show_input_email')) {
            ...
            for ($i = 0; $i < count($bids); $i++) {
                ...
                echo $database->getErrorMsg();
                $bookid = _BOOKLIBRARY_LABEL_BOOKID;
                $book_isbn = _BOOKLIBRARY_LABEL_ISBN;
                if (trim($item_book[0]->owneremail) != '')
                    $mail_to[] = $item_book[0]->owneremail;

                /*  if (count($mail_to) > 0) {
                  if ($userid == 0) {
                  mosMail($mosConfig_mailfrom, 'anonymous', $mail_to, 'New lend request added!', 'User anonymous has submitted a Lend Request for:<br /><br /> ' .
                  $bookid . ': ' . $item_book[0]->bookid . '<br />' .
                  $book_isbn . ': ' . $item_book[0]->isbn . '<br />' .
                  $item_book[0]->title . '<br /><br /> ' .
                  'Please log on and approve or deny this Lend Request', true);
                  } else {
                  mosMail($mosConfig_mailfrom, $item_user[0]->name, $mail_to, 'New lend request added!', 'User ' . $item_user[0]->name . ' has submitted a Lend Request for:<br /><br /> ' .
                  $bookid . ': ' . $item_book[0]->bookid . '<br />' .
                  $book_isbn . ': ' . $item_book[0]->isbn . '<br />' .
                  $item_book[0]->title . '<br /><br /> ' .
                  'Please log on and approve or deny this Lend Request', true);
        ...
        //-------------------------------
        /*
          User {username} has submitted a Lend Request for:
        ...
        if ($booklibrary_configuration['lendrequest_email']['show'] && trim($booklibrary_configuration['lendrequest_email']['address']) != "") {
            ...
            //mail($booklibrary_configuration['lendrequest_email']['address'], $subject, $message, $headers);
            mosMail($_REQUEST['user_email'], $_REQUEST['user_name'], $booklibrary_configuration['lendrequest_email']['address'], $subject, $message);
        }
        //-------------------------------
...
// 	$params = new mosParameters( $menu->params );
        ...
        if ($params->get('show_input_email')) {
            ...
            $t = _BOOKLIBRARY_LABEL_TITLE_COMMENT; //Review title
            $c = _BOOKLIBRARY_LABEL_TITLE_REVIEW_COMMENT; //Review comment
            $r = _BOOKLIBRARY_LABEL_RATING; //Rating
            $rating = (($item_review[0]->rating) / 2);

            /* if ($item_review[0]->fk_userid == 0) {
              mosMail($mosConfig_mailfrom, 'anonymous', $mail_to, 'New book review added', 'User Anonymous has added a Review to book:<br />' .
              $book_title[0]->title . '<br /><br />' .
              $t . ': "' . $item_review[0]->title . '"<br /><br /> ' .
              $r . ': [' . $rating . ']<br /><br /> ' .
              $c . ':<br /> ' . $item_review[0]->comment . '<br /><br /> ' .
              'Please log on and edit this review where necessary.', true);
              } else if ($item_review[0]->fk_userid != 0) {
              mosMail($mosConfig_mailfrom, $item_user[0]->name, $mail_to, 'New book review added', 'User ' . $item_user[0]->name . ' has added a Review to book:<br />' .
              $book_title[0]->title . '<br /><br />' .
              $t . ': "' . $item_review[0]->title . '"<br /><br />  ' .
              $r . ': [' . $rating . ']<br /><br / ' .
              $c . ':<br /> ' . $item_review[0]->comment . '<br /><br /> ' .
            ...
            //-----------------------
            /*
              User {username} has added a Review to book:<br />{book_title}<br /><br />
              {LABEL_TITLE_COMMENT}: {title}<br /><br />
              {LABEL_RATING}: [{rating}]<br /><br />
              {LABEL_TITLE_REVIEW_COMMENT}:<br /> {comment} <br /><br />
              Please log on and edit this review where necessary.');
             */

            if (/* $booklibrary_configuration['review_email']['show'] && */ trim($booklibrary_configuration['review_email']['address']) != "") {
                $query = "SELECT * FROM #__booklibrary WHERE id='" . $_REQUEST['bookid'] . "'";
                $database->setQuery($query);
                $book_name = $database->loadAssoc();
                ...
                $message = str_replace("{username}", $name, $message);
                $message = str_replace("{book_title}", $book_name['title'], $message);
                $message = str_replace("{LABEL_TITLE_COMMENT}", _BOOKLIBRARY_LABEL_TITLE_COMMENT, $message);
                $message = str_replace("{title}", $_REQUEST['title'], $message);
                $message = str_replace("{LABEL_RATING}", _BOOKLIBRARY_LABEL_RATING, $message);
                $message = str_replace("{rating}", $_REQUEST['rating'], $message);
                $message = str_replace("{LABEL_TITLE_REVIEW_COMMENT}", _BOOKLIBRARY_LABEL_TITLE_REVIEW_COMMENT, $message);
        ...
        mosRedirect("index.php?option=com_booklibrary&view=categories&layout=categories&Itemid=174");
        //mosRedirect("index.php?option=com_booklibrary&task=view&catid=".intval($_POST['catid'])."&id=$review->fk_bookid&Itemid=$Itemid");
    }
...
//this function check - is exist folders under this category 
    static function is_exist_subcategory_books($catid) {
        ...
        foreach ($categories as $k) {
            if (is_exist_subcategory_books($k->id))
                return true;
        }
        return false;
    }
...
//end function
    ...
    static function listCategories($catid) {
        ...
        $query = "SELECT COUNT(bc.bookid) as books " .
            ...
            \n WHERE  c.section='com_booklibrary'
            \n AND b.published=1 AND b.approved=1 AND ({$s}) AND (c.langshow LIKE'" . $lang1 . "' OR c.language='' OR c.language IS NULL OR c.language='*')
...
//       }
        ...
        LEFT JOIN #__booklibrary AS b ON b.id=bc.bookid AND b.published=1 AND b.approved=1
        WHERE  c.section='com_booklibrary'
          AND c.published = 1 AND ({$s}) AND (c.langshow LIKE'" . $lang1[0] . "' OR c.language='' OR c.language IS NULL OR c.language='*')
        GROUP BY c.id \n
        ORDER BY parent_id DESC, c.ordering ";
        $database->setQuery($query);
        $cat_all = $database->loadObjectList();
        ...
        $query = "SELECT COUNT(bc.bookid) as books " .
            ...
            \n LEFT JOIN #__booklibrary AS b ON b.id=bc.bookid 
            \n WHERE  c.section='com_booklibrary'
            \n AND b.published=1 AND b.approved=1 AND ({$s}) AND (c.langshow LIKE'" . $lang1[0] . "' OR c.language='' OR c.language IS NULL OR c.language='*')
            \n AND c.id = " . $cat_all[$k1]->id . "    
            \n GROUP BY c.id
            \n ORDER BY c.parent_id DESC, c.ordering ";              
